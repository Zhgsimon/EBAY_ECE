...
///si j'ai appuyé sur le bouton achat immédiat
if ($_GET['submit_action']) {
  $pic1=$_GET['submit_action'];
  $type_achat=$_GET['type_achat'];

  //je récup dans la BDD le prix de l'item
  $stmt = $bdd->prepare("SELECT prix_immediat FROM item WHERE pic1=? LIMIT 1");
  $stmt->execute(array($pic1));
  $row = $stmt->fetch();

  $somme_a_paye=$row['prix_immediat'];

}

//je récupère le prix à Payer de la methode get
//

/*$req=$bdd->query('SELECT * FROM item WHERE etat_vente="en_vente" ORDER BY ID_item DESC LIMIT '.$premiereEntree.', '.$nombre_item_par_page.'');
...
input[type=number]::-webkit-outer-spin-button {
  ...
  $("#radio_new_card").click(function(){

  $("#old_card").css("display", "none");
  $("#new_card").css("display", "block");
  //$("#duree_vente_enchere").css("display", "block");
});

$("#old_card").click(function(){

$("#old_card").css("display", "block");
...
<body>
  ...
  <div class="container">
    ...
    <h3 class="headingTop text-center">Saisissez vos coordonées bancaires</h3>
    <!--<input type="radio" id="radio_new_card" name="card" onclick="show_new_card();"  value="New_card">Payer avec une nouvelle carte</input>
    <br>

    <input type="radio" id="radio_old_card" name="card" onclick="notshow_new_card();" value="default_card"></input>Payer avec la carte enregistrée</div>-->
    <!--On ajoute les infos bancaire d'une nouvelle carte-->
    <form role="form" method="post" action="traitement_paiement.php">
    <div id="new_card" >
      <div class="row">
